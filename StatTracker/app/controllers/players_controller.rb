...
autocomplete :player, :name, :full => true
  ...
  def show
    ...
    query = "\"#{@player.name}\" \"baseball card\" \"#{@player.fielding_stats.last.team.year}\""
    #throw query
  ...
  puts query
    ...
    numberrbi=BattingStat.get_stat_total(params[:id], :rbi)
		 ...
		 @chart.set_cell(i, 18,  @player.career_total_bases.to_i, "<span title=#{bases}>#{@player.career_total_bases}</span>")
